...
contract DepositManager {
    ...
    function removeDeposit(uint256 _index) internal {
        ...
        if (_index == 0) {
            // uint256 numberOfDeposits = pendingDeposits.length; // check - v0.6.0
            // for (uint256 i = 0; i < numberOfDeposits; i++) { // check - v0.6.0
            //     delete pendingDeposits[i]; // check - v0.6.0
            // } // check - v0.6.0
            // pendingDeposits.length = 0; // check - v0.6.0
            delete pendingDeposits; // check - v0.6.0
            return;
        }

        if (_index == pendingDeposits.length - 1) {
            // delete pendingDeposits[pendingDeposits.length - 1]; // check - v0.6.0
            // pendingDeposits.length--; // check - v0.6.0
            pendingDeposits.pop(); // check - v0.6.0
            return;
        }
    }
}
